...
class PineconeDB_VectorStore_V2(VannaBase):
    """
    ...
    Args:
        config (dict): Configuration dictionary. Defaults to {}. You must provide either a Pinecone Client or an API key in the config.
            ...
            - dimensions (int, optional): Dimensions of the embeddings. Defaults to 1536 which corresponds to OpenAI's text-embedding models.
            - embedding_model (str, optional): OpenAI embedding model to use. Defaults to "text-embedding-3-small".
            - embedding_client (OpenAI, optional): OpenAI client for generating embeddings.
            ...
            - podspec (PodSpec, optional): PodSpec configuration if using a pinecone pod.
            - serverless_spec (ServerlessSpec, optional): ServerlessSpec configuration if using a pinecone serverless index.
    Raises:
    ...
    def __init__(self, config=None):
        ...
        self.dimensions = config.get("dimensions", 1536)  # OpenAI embedding models use 1536 dimensions
        ...
        if not self.embedding_client:
            raise ValueError("embedding_client is required in config for embedding generation")
    ...
    def _check_if_embedding_exists(self, id: str, namespace: str) -> bool:
        ...
        return bool(fetch_response.get('vectors', {}))
    ...
    def get_related_ddl(self, question: str, **kwargs) -> list:
        ...
        return [match.metadata["ddl"] for match in matches] if matches else []
    ...
    def get_related_documentation(self, question: str, **kwargs) -> list:
        ...
        if hasattr(res, 'matches'):
            ...
            matches = res.get('matches', [])
        return [match.metadata["documentation"] for match in matches] if matches else []
    ...
    def get_training_data(self, **kwargs) -> pd.DataFrame:
        ...
        for data_type, namespace in namespaces.items():
            ...
            if matches:
                ...
                content_list = [
                    match.metadata[data_type] for match in matches
                ]
                question_list = [
                    (
